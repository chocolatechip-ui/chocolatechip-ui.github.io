...
(function(DOMStack) {
  ...
  /**
   * ChocolateChipJS collection extend method.
   */
  $.fn = {
  ...
  /**
   * ChocolateChipJS utility methods.
   */
  $.extend({
    lib: "ChocolateChipJS",
    ...
    require: function require(src, callback, ctx) {
      ...
      var setupLoad = function setupLoad(fn) {
        return function() { // Only call once.
          if (done) {
            return;
          }
          done = true;
    ...
    compare: function compare(value1, value2) {
      function compareNativeSubtypes(value1, value2) { // e.g. Function, RegExp, Date
  ...
  $.extend({ // Set up subscriber:
    on: function on(topic, handler) { // const self = this;
      // Create the topic's object:
      if (!hasProp.call(topics, topic)) topics[topic] = []; // Add subscriber:
      var index = topics[topic].push(handler) - 1; // Return method to delete subscriber:
      return {
        off: function off() {
          return delete topics[topic][index];
        }, // Return method to run this subscriber -
        // `mySubscriber.run('foo', myDataHere)`
    ...
    send: function send(topic, payload) {
      if (!hasProp.call(topics, topic)) return; // Loop through topics and execute:
  ...
  /**
   * ChocolateChipJS string methods.
   */
  $.extend({
  ...
  $.fn.extend({
    ...
    is: function is(arg) {
      ...
      var __is = function __is(node, arg) {
        if (typeof arg === 'string') {
          ...
          if (_this3.slice.apply(arg).indexOf(node) !== -1) {
            ret = true;
          }
    ...
    has: function has(arg) {
      ...
      var __has = function __has(node, arg) {
        if (typeof arg === 'string') {
          ...
          if (Array.prototype.slice(_this4.children).data.indexOf(arg)) {
            return true;
          }
        } else {
          return false;
    ...
    siblings: function siblings(selector) {
      ...
      var $this = this;
      var parent = void 0;
      var children = Array.prototype.slice.apply(this.array[0].parentNode.children); // Remove this from siblings;
  ...
  /**
   * ChocolateChipJS serialize methods.
   */
  $.fn.extend({
  ...
  /**
   * ChocolateChipJS param method.
   */
  $.extend({ // Serialize an object for posting to server:
    //===========================================
    param: function param(obj, traditional) { // Private function used by $.param:
      //==================================
      function serialize(params, obj, traditional, scope) {
        var type = void 0,
          array = $.type(obj) === 'array',
          hash = $.isEmptyObject(obj); // If it's an array of objects:
        if ($.type(obj) === 'array') {
  ...
  (function() {
    ...
    // ======================================
    var ChuiEventStack = function ChuiEventStack(array) {
      var __array = [];
    ...
    /* jshint, evil: false, validthis:true, unused:false, loopfunc: false,
        smarttabs: true, nonew: false */ // Private method to set events on CCEventCache
    //=============================================
    var bind = function bind(element, event, callback, capturePhase) {
      if (!element.id) element.id = chocolatechipjs.uuid();
      if (!CCEventCache.elements[element.id]) {
        CCEventCache.elements[element.id] = ChuiEventStack(); // jshint ignore:line
      }
      CCEventCache.elements[element.id].push({
        event: event,
        callback: callback
      });
    ...
    }; // Private method to unbind events on CCEventCache
    ///===============================================
    var unbind = function unbind(element, event, callback, capturePhase) {
      var eventStack = CCEventCache.elements[element.id];
    ...
    }; // Set delegated events on CCEventCache
    //=====================================
    ...
    }; // Method to remove delegated events from CCEventCache
    //====================================================
  ...
  /**
   * ChocolateChipJS event aliases.
   */
  $.extend({
    eventStart: null,
    eventEnd: null,
    eventMove: null,
    eventCancel: null, // Define min-length for gesture detection:
    gestureLength: 30
  });
  $(function() { //=======================
    ...
    if (window.navigator.pointerEnabled) {
      ...
      $.eventCancel = 'pointercancel'; // Pointer events for IE10 and WP8:
    } else if (window.navigator.msPointerEnabled) {
      $.eventStart = 'MSPointerDown';
      $.eventEnd = 'MSPointerUp';
      $.eventMove = 'MSPointerMove';
      $.eventCancel = 'MSPointerCancel'; // Touch events for iOS & Android:
    } else if ('ontouchstart' in window && /mobile/img.test(navigator.userAgent)) {
      $.eventStart = 'touchstart';
      $.eventEnd = 'touchend';
      $.eventMove = 'touchmove';
      $.eventCancel = 'touchcancel'; // Mouse events for desktop:
    } else {
  ...
  (function() {
    ...
    $(function() {
      ...
      var delta = void 0;
      var body = $('body');
      var twoTouches = false; // Capture start of event:
      //========================
      body.on($.eventStart, function(e) {
        now = Date.now();
        delta = now - (touch.last || now);
        if (e.originalEvent) e = e.originalEvent; // Handle MSPointer Events:
        if (window.navigator.pointerEnabled || window.navigator.msPointerEnabled) {
          ...
          if ($.eventStart === 'mousedown') {
            ...
            touch.x1 = e.pageX;
            touch.y1 = e.pageY;
            twoTouches = false; // Detect two or more finger gestures:
          } else {
      ...
      body.on($.eventEnd, function(e) {
        ...
        if (!!touch.el) { // Swipe detection:
          if (touch.x2 && Math.abs(touch.x1 - touch.x2) > $.gestureLength || touch.y2 && Math.abs(touch.y1 - touch.y2) > $.gestureLength) {
            ...
            tapTimeout = setTimeout(function() { // Trigger double tap immediately:
              if (touch && touch.isDoubleTap) {
                if (touch && touch.el) {
                  touch.el.trigger('doubletap');
                  touch = {};
                }
              } else { // Trigger tap after singleTapDelay:
  ...
  /**
   * ChocolateChipJS collection utilities.
   */
  $.fn.extend({
    forEach: function forEach(callback) {
  ...
  $.fn.extend({
    ...
    validatePhoneNumber: function validatePhoneNumber(int) {
      ...
      if (this[0].value) { // International Numbers:
        if (int) {
          phoneNumber = this[0].value.replace(/[\(\)\.\-\ ]/g, '');
          return checkValidity(this, this.isNotEmpty() && !isNaN(phoneNumber)); // North America (US and Canada):
        } else {
          phoneNumber = this[0].value.replace(/[\(\)\.\-\ ]/g, '');
  ...
  $.extend({ //===========================================
    ...
    Form: function Form(options) {
      ...
      var __passed = false;
      var __errors = [];
      var __result = []; // Helper to validate form elements:
      //==================================
      ...
      options.forEach(function(item) {
        ...
        switch (item.type) {
          ...
          case 'username':
            ...
            validateElement(item); // minimum length
            return;
          case 'email':
          ...
          case 'checkbox':
            ...
            if (__passed) { // var checkbox = $(item.element)[0];
              validateElement(item);
            }
            return;
          case 'radio':
      ...
      return {
        ...
        get: function get() { // console.log(__result)
  ...
  $.extend({ //==============================
    ...
    formatNumber: function formatNumber(amount, separator, decimal) {
      var sep = separator || ","; // Allow the user to round a float to a whole number:
      if (decimal === 0) {
        var num = Math.round(amount);
        return Number(num).toString().replace(/(?=(?:\d{3})+$)(?!^)/g, sep);
      }
      if (decimal === undefined) { // Check if amount is a float:
        if (typeof amount === 'number' && amount % 1 !== 0) {
          return Number(amount).toString().replace(/\d(?=(\d{3})+\.)/g, '$&' + sep); // Otherwise treat it as an integer:
        } else {
    ...
    currency: function currency(amount, symbol, separator, decimal) {
      ...
      var formatNumber = function formatNumber(amount, sep) { // A decimal value of '0' means
        ...
        } else { // Otherwise, we can just add the thousands
    ...
    View: function View(options) {
      ...
      $.extend(view, {
        ...
        render: function render(data, append) {
          ...
          if (!data && !__data && !__model) { // if (data === 0) return;
          ...
          if ($.type(data) === 'array') {
            ...
            $.view.index = 0;
            __element.removeClass('cloak');
            return; // Else if it is an object:
...
(function() { //==================================
  ...
  function isThenable(obj) {
    var _then = void 0,
      obj_type = typeof obj === 'undefined' ? 'undefined' : _typeof(obj);
  ...
  extend(Promise, "prototype", PromisePrototype, false); // Check if Promise is initialized:
  extend(PromisePrototype, "isValidPromise", 0, false);
  extend(Promise, "resolve", function(msg) {
    var Constructor = this; // Make sure it is a valide Promise:
  ...
  extend(Promise, "all", function(arr) {
    var Constructor = this; // Make sure argument is an array:
...
(function() {
  ...
  $.extend({ // Container for jsonp methods:
    ...
    jsonp: function jsonp(url, opts) {
      ...
      var callbackName = generateCallbackName(); // Create and return Promise with result from request:
      return new Promise(function(resolve, reject) {
        var timeout;
        ...
        document.body.appendChild(script); // Delete script tag:
        setTimeout(function() {
...
if (!Array.prototype.unique) {
  ...
  $.extend({
    Model: function Model(data) {
      ...
      var mod = this;
      mod.data = undefined;
      mod.boundViews = []; // Views get bound to a model in a view initialization.
      ...
      function createObjectModel(data) {
        ...
        return _ref = {
          id: __id,
          data: data, // Set model state to `stopped`.
          // Used to determine whether to 
          // execute a model event.
          stop: function stop() {
          ...
          // This will not replace any existing
          // properties of the same name.
          ...
          mixin: function mixin(obj) {
            ...
            for (var key in obj) { // Do not replace property if it exists:
              if (!(key in mod.data)) {
      ...
      if (data) mod.data = data; // Define an object-based model:
      if ($.type(data) === 'object') {
        return createObjectModel(data); // Define an array-based model:
      } else if ($.type(data) === 'array') {
        return {
          id: __id, // Set model state to `stopped`.
          // Used to determine whether to 
          // execute a model event.
          stop: function stop() {
            return __stopped = true;
          ...
          }, // Push an object to the model's collection:
          push: function push(data) {
            if (!data) {
            ...
            mod.data.push(data);
            updateBoundViews(mod);
          }, // Pop the last object from the model:
          pop: function pop() {
            mod.data.pop();
            updateBoundViews(mod);
          }, // Pop the first object in the model:
          ...
          orderBy: function orderBy() {
            ...
            var orderBy = function orderBy(args) {
              return function(a, b) {
                var sortByProperty = function sortByProperty(property) { // Default sort order:
                  var sortOrder = 1; // If user provided property 
                  // with "-" prefix, make
                  // sort order descending:
                  if (property[0] === "-") {
                    sortOrder = -1; // Extract property from hyphen prefix:
                ...
                while (result === 0 && i < numberOfProperties) { // Use the private function to compare two values:
                  //================================================
        ...
        // so define a default object-based model:
      } else if (!data) {
        var _data = {};
...
$.extend($, {
  ...
  isMSEdge: /Edge\/\d./i.test(navigator.userAgent),
...
$.extend({ // Create a color object from provided color value.
  ...
  ChuiColor: function ChuiColor(color) {
    this.ok = false; // console.log(color)
    // Check for HEX color value.
    // String "#" if found:
    if (color.charAt(0) == '#') {
      color = color.substr(1, 6);
    }
    color = color.toLowerCase(); // Array of color format regex patterns.
  ...
  // Original values: ((r * .299) + (g * .587) + (b * .114))
  // We changed this to increase contrast for areas between 
  // increased lightness and desaturation. 
  // This method expects a color object created by $.ChuiColor.
  //===========================================================
  getBrightness: function getBrightness(color) {
    if ($.type(color) === 'string') color = new $.ChuiColor(color);
...
}); // ChocolateChipUI Widget - Activity Indicator:
$.fn.extend({ //===========================
  ...
  Busy: function Busy(options) {
    ...
    var iOSBusy = function iOSBusy() {
      ...
      if (parseInt(settings.size, 10) < 30) {
        ...
        spinner = '<svg class=\'chui-busy\' width=\'' + settings.size + 'px\' height=\'' + settings.size + 'px\' xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 100 100\' preserveAspectRatio=\'xMidYMid\' class=\'uil-default\'> \n  <g x=\'0\' y=\'0\' width=\'100\' height=\'100\' fill=\'none\'>\n    <rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(0 50 50) translate(0 -30)\'></rect><rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(30 50 50) translate(0 -30)\'></rect>\n    <rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(60 50 50) translate(0 -30)\'></rect><rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(90 50 50) translate(0 -30)\'></rect>\n    <rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(120 50 50) translate(0 -30)\'></rect><rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(150 50 50) translate(0 -30)\'></rect>\n    <rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(180 50 50) translate(0 -30)\'></rect><rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(210 50 50) translate(0 -30)\'></rect>\n    <rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(240 50 50) translate(0 -30)\'></rect><rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(270 50 50) translate(0 -30)\'></rect>\n    <rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(300 50 50) translate(0 -30)\'></rect><rect  x=\'46.5\' y=\'40\' width=\'7\' height=\'20\' rx=\'5\' ry=\'5\' fill=\'' + settings.color + '\' transform=\'rotate(330 50 50) translate(0 -30)\'></rect>\n  </g>\n</svg>';
        $this.append(spinner);
      }
    }; // For Android:
    ...
    var winBusy = function winBusy() {
      var spinner = $('<progress class="chui-busy"></progress>');
      $(spinner).css({
...
}); // ChocolateChipUI Widget - Screen Blocker:
$.extend({ //==============
  // Cover screen:
  //==============
...
}); // ChocolateChipUIs Widget - Button functions:
...
}); // ChocolateChipUI Widget - Center Elements:
$.fn.extend({ //============================
...
}); // ChocolateChipUI Widget - Edit List:
$.extend({ //=================================
  ...
  EditList: function EditList(options) {
    ...
    var setupDeletability = function setupDeletability(callback, list, button) {
      $(function() { // Check for view and update its template:
        if (__view) {
          var temp = $('<div></div>');
        ...
        button.on('tap', function() {
          var $this = this; // When button is in "Edit" state:
          if (this.classList.contains('edit')) {
            ...
            $(list).removeClass('editing'); // Execute callback if edit was performed:
            //========================================
            setTimeout(function() {
        ...
        $(list).on(dispelDeletable, 'li', function() { // If no deletables, disable swipes:
          if (!settings.deletable) return; // Else reveal delete button:
          $(this).removeClass('selected');
        });
        $(list).on(enableDeletable, 'li', function() { // If no deletables, disable swipes:
          if (!settings.deletable) return; // Else reveal delete button:
        ...
        $(list).on('tap', '.move-up', function(e) {
          ...
          if (item.is('li:first-child')) {
            ...
            (function() { // Mark list as edited:
              $(list).data('list-edit', true);
              item = $(_this).closest('li');
              var prev = item.prev(); // Clone the items to replace the
        ...
        $(list).on('tap', '.move-down', function(e) {
          ...
          if (item.is('li:last-child')) {
            ...
            (function() { // Clone the items to replace the
              // transitioned ones alter:
              var itemClone = item.clone();
              var nextClone = next.clone(); // Mark list as edited:
              $(list).data('list-edit', true);
              var height = item[0].offsetHeight;
              item.css({
                '-webkit-transform': 'translate3d(0,' + height + 'px,0)',
        ...
        $(list).on('tap', '.delete', function() {
          var $this = this;
          var listItem = $(this).parent(); // Mark list as edited:
          $(list).data('list-edit', true);
          var direction = '-1200%';
...
}); // ChocolateChipUI Widget - Multi-Select List:
$.extend({ //===========================
    ...
    list.on('tap', 'li', function() {
      ...
      if (item.hasClass('selected')) {
        (function() {
          ...
          settings.callback.apply(_this8, _arguments2);
        })();
      } else {
        __selection.push({
          index: item.index(),
...
}); // ChocolateChipUI Widget - Adjust Navbar for iOS:
$(function() { //===============================
  ...
  $.extend({
    AdjustNavbarLayout: function AdjustNavbarLayout(screen) {
      ...
      if (siblings.length === 1) {
        handleOneSibling(); // If two siblings:
...
}); // ChocolateChipUI Widget - Navigation Module:
$(function() { // Private variable to track navigation state:
  var isNavigating = false; // get screen by id:
...
$.extend({ //========================
  ...
  Paging: function Paging(options) {
    ...
    $.AdjustNavbarLayout(screen); // Handle paging forward:
    var pageForward = function pageForward(pagerButton) {
      ...
      if (currentArticle.index() === articles() - 1) { // start again!
...
}); // ChocolateChipUI Widget - Popover:
$.extend({ //=================================
  ...
  Popover: function Popover(options) {
    ...
    var popoverID = '#' + settings.id; // Calculate position of popover relative to the button that opened it:
...
$(function() { /////////////////////////////////////////////////
  ...
  $('body').on(events, '.mask', function(e) {
    if (!$('.popover')[0]) {
      if (e && e.nodeType === 1) return; // e.stopPropogation();
    } else {
      $.ClosePopover();
    }
  });
}); // ChocolateChipUI Widget - Popup:
$.extend({ //=======================
  ...
  Popup: function Popup(options) {
    ...
    if (callback && continueButton) {
      $('#' + id).find('.continue').on($.eventStart, function() {
        var $this = $(this);
        if ($.isAndroid || $.isChrome) {
          $this.addClass('selected');
          setTimeout(function() {
      ...
      $('#' + id).find('.cancel').on($.eventStart, function() {
        var $this = $(this);
        if ($.isAndroid || $.isChrome) {
          $this.addClass('selected');
          setTimeout(function() {
...
}); // ChocolateChipUI Widget - Range Input:
$.fn.extend({ //=====================
  // Setup a range input:
  //=====================
        ...
        if (baseRouteOnly) {
          ...
          $.ChuiRoutes.forEach(function(r) {
            if (r && route === r.split(':')[0]) {
              var index = $.ChuiRoutes.indexOf(r);
              $.ChuiRoutes.splice(index, 1);
            }
          });
        }
...
$(function() { // Set up initial route:
  if ($('screen').size() && !$.ChuiRoutes.length) {
    $.ChuiRoutes.push($('screen')[0].id);
  }
}); // ChocolateChipUI Widget - Screens:
$(function() { //=================================
...
}); // ChocolateChipUI Widget - Segmented Buttons:
$(function() {
  $.extend({ //==========================
    ...
    Segmented: function Segmented(options) {
      ...
      createSegmentedButton(); // For Android Material Design:
      ...
      $(__element).on('tap', '.segmented > button', function(e) {
        ...
        callback.call(this, e); // For Android Material Design:
        if ($('body').hasClass('isAndroid')) {
...
}); // ChocolateChipUI Widget - Select List:
$.extend({ //=====================
  // Setup a select list:
  //=====================
...
$.extend({ //========================
  ...
  SlideOut: function SlideOut() {
    ...
    $('body').append(slideOutButton); // Get Slide Out & Slide Out button:
    var slideout = $('#' + slideoutID);
    var slideOutBtn = $('#' + buttonID); // Set up state for Slide Out and screens:
    slideout.attr('aria-hidden', true);
    $("screens").attr('aria-hidden', true);
    $("screens").eq(0).addClass('show').attr('aria-hidden', "false"); // Handle Slide Out button events:
    ...
    $('#' + slideoutID).on('tap', 'li', function() {
      ...
      slideout.attr('aria-hidden', 'true'); // Toggle Slide Out button:
      slideOutBtn.toggleClass('focused');
      $('button.back').removeClass('disabled').removeProp('disabled');
      $('button.backTo').removeClass('disabled').removeProp('disabled'); // This list item shows a single screen:
      ...
      $('screen.previous').addClass('next').removeClass('previous'); // Get route to dispatch:
      $.Router.dispatch(screenToShow);
      if ($.ChuiRoutes.length > 1) $.ChuiRoutes.pop(); // Close slide out:
      slideout.removeClass('open');
    });
    return {
...
}); // ChocolateChipUI Widget - Stepper:
$.extend({ //==================
  // Create a stepper:
  //==================
...
}); // ChocolateChipUI Widget - Switches:
$.extend({ //=========================
  ...
  Switch: function Switch(options) {
    ...
    var __element = $(settings.element);
    __checked = settings.checked; // Abrstract swipe for left-to-right and right-to-left:
...
}); // ChocolateChipUI Widget - Tab Bar:
$.extend({ //=========================================
  ...
  TabBar: function TabBar(options) {
    ...
    var showIcons = settings.showIcons ? ' class="showIcons" ' : ''; // Helper: Set Screen to Current:
    //===============================
    ...
    $('body').prepend(tabbarTmpl); // Get id of appended tab bar:
    var tabbar = $('#' + settings.id); // Private variables to manage tab bar: 
    var __tabbarButtons = tabbar.find('button');
    var __selectedTabbarButton = __tabbarButtons.eq(settings.selected);
    var __selectedTabbarScreen = __tabbarScreens.eq(settings.selected); // Setup events on tabs:
    //======================
    $(function() {
      $.ChuiRoutes = [];
      ...
      tabbar.on('tap', 'button', function() {
        ...
        var navRoutes = navRoutesFull.split('/'); // This tab holds a navigation list:
        if (routes.length > 1) {
          ...
          navRoutesFull = $(this).data('chui-route'); // Set this tab to `selected`:
          tab.addClass('selected'); // Deal with previously selected tab and screen:
          ...
          $.ChuiRoutes = routes;
          $.Router.dispatch(routes[routes.length - 1]); // This tab has a single screen:
        } else {
          var screen = $('screen').eq(tab.index());
          __tabbarButtons.hazClass('selected').data('chui-route', fullRoute);
          __tabbarButtons.hazClass('selected').removeClass('selected').addClass('next'); // Deal with previously selected tab and screen:
          $('screen').removeClass('current').addClass('next').attr('aria-hidden', true);
          $('screen').removeClass('previous').addClass('next').attr('aria-hidden', true); // Set this tab to `selected`:
          tab.addClass('selected'); // Make this tab's screen `current`:
...
$(function() {
  if ($.theme === 'android' && !$.isDesktop) {
    $('body').on($.eventStart, 'button, ul.editing > li, li[data-show], li[data-select], span.move-down, span.move-up, li[data-goto]', function(event) {
      var $ripple = $('<ripple></ripple>'); // Get offsets:
      var elementOffset = $(this).offset();
      var xPos = event.pageX - elementOffset.left;
      var yPos = event.pageY - elementOffset.top; // Handle ripple color:
      var rippleColor = '#fff';
      var backgroundColor = $(this).css('background-color');
      if (!backgroundColor || backgroundColor === 'rgba(0, 0, 0, 0)') backgroundColor = '#fff';
      var brightness = $.calculateBrightness(backgroundColor);
      if (brightness < 210 && brightness > 195) {
        rippleColor = '#bbb';
      } else if (brightness > 195) {
        rippleColor = '#ccc';
      } else {
        rippleColor = '#fff';
      } // rippleColor = (brightness > 195) ? "#ccc" : "#fff";
      if ($(this)[0].dataset.rippleColor) {
        rippleColor = $(this)[0].dataset.rippleColor;
      }
      $(this).prepend($ripple); // Set ripple styles:
      $ripple.css("height", $(this).height() + 'px');
      $ripple.css("width", $(this).height() + 'px');
      $ripple.css({
        top: yPos - $ripple.height() / 2 + 'px',
        left: xPos - $ripple.width() / 2 + 'px',
        'background-color': rippleColor
      });
      if ($(this).hasClass('move-down') || $(this).hasClass('move-up')) {
